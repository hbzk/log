...
function dragdrop_drop() {
	$('#start').droppable(	{tolerance : 'touch'	},{accept : '.drag'}
	...
	db.transaction(function(tx) {
		tx.executeSql('SELECT * FROM ACTION', [], function(tx, res) {
			for (var i = 0; i < res.rows.length; i++) {
				...
				if (row.POSITION > 0) {
					$($(iconDiv)[row.POSITION - 1])
						.append("<i data-name = '" + row.ICON_NAME + "' class = '" 	+ row.CLASS_NAME + "'></i>");
				}
			}
...
function db_endQuery() {
	...
	db.transaction(function(tx) {
		...
		tx.executeSql('UPDATE LOG SET END_TIME=?, DURATION=? WHERE ID=?'
				, [localISOString(endTime), resultWhile, lastRow.ID]);
	}, db_errorCB);
}
...
function showConfirm() {
	...
	navigator.notification.confirm('알림을 종료할까요?', // message
